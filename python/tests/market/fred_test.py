...
class TestGetFredClient:
    ...
    @patch("portfolioos.market.fred.Fred")
    def test_explicit_api_key(self, mock_fred_cls):
        _get_fred_client(api_key="test-key-123")
        mock_fred_cls.assert_called_once_with(api_key="test-key-123")

    @patch("portfolioos.market.fred.Fred")
    def test_env_var_api_key(self, mock_fred_cls):
        with patch.dict("os.environ", {"FRED_API_KEY": "env-key-456"}):
            _get_fred_client(api_key=None)
...
class TestFetchSeries:
    ...
    def test_returns_list_of_dicts(self, mock_get_client):
        ...
        result = fetch_series("FEDFUNDS", "2024-01-01", "2024-06-30", api_key="test")
        assert isinstance(result, list)
        assert len(result) == 3
        assert all(isinstance(r, dict) for r in result)

    @patch("portfolioos.market.fred._get_fred_client")
    def test_contains_required_fields(self, mock_get_client):
        mock_client = MagicMock()
        mock_get_client.return_value = mock_client
        ...
        result = fetch_series("CPIAUCSL", "2024-01-01", "2024-03-31", api_key="test")
    ...
    @patch("portfolioos.market.fred._get_fred_client")
    def test_nan_values_dropped(self, mock_get_client):
        mock_client = MagicMock()
        mock_get_client.return_value = mock_client
    ...
    @patch("portfolioos.market.fred._get_fred_client")
    def test_series_id_uppercased(self, mock_get_client):
        mock_client = MagicMock()
        mock_get_client.return_value = mock_client
...
class TestFetchMultipleSeries:
    ...
    def test_fetches_all_series(self, mock_fetch):
        ...
        result = fetch_multiple_series(
            ["FEDFUNDS", "UNRATE"], "2024-01-01", "2024-06-30", api_key="test"
        )
        assert len(result) == 2
    ...
    def test_failed_series_omitted(self, mock_fetch):
        def side_effect(series_id, *args, **kwargs):
            ...
            return [{"date": "2024-01-01", "value": 1.0, "series_id": series_id}]

        mock_fetch.side_effect = side_effect

        result = fetch_multiple_series(
            ["FEDFUNDS", "BAD", "UNRATE"], "2024-01-01", "2024-06-30", api_key="test"
        )
        assert len(result) == 2
